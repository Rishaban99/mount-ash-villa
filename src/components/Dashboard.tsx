...
import { DashboardStats, RecentActivity, User, Room } from '../types';
import {
  Hotel,
  CheckCircle,
  AlertCircle,
...
interface DashboardProps {
  currentUser: User;
  onNavigateTab: (tab: string, arg?: string) => void;
  refreshTrigger: number;
}

export const Dashboard: React.FC<DashboardProps> = ({ currentUser, onNavigateTab, refreshTrigger }) => {
      ...
      <div className="bg-slate-900 text-white p-5 rounded-2xl border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-md relative overflow-hidden">
        ...
        <div className="relative z-10 shrink-0 self-start md:self-center">
          <button
            ...
            type="button"
            onClick={() => onNavigateTab('expenses', 'new')}
